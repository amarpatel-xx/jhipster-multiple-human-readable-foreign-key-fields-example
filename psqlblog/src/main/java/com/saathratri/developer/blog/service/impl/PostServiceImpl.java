...
public class PostServiceImpl implements PostService {
    ...
    public List<PostDTO> findAllForSaathratriOrchestrator() {
        LOG.debug("Request to get all Posts for saathratri orchestrator");
    ...
    @Override
    @Transactional(readOnly = true)
    public Optional<PostDTO> findOneWithEagerRelationships(UUID id) {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PostDTO> findAllWithEagerRelationships() {
        LOG.debug("Request to get all Posts");

        List<Post> posts = postRepository.findAllWithEagerRelationships();
        List<PostDTO> postDTOList = new java.util.ArrayList<PostDTO>(posts.size());

        for (Post postDTO : posts) {
            postDTOList.add(postMapper.toDto(postDTO));
        }
