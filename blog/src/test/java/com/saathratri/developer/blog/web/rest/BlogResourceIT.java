...
class BlogResourceIT {
    ...
    void getAllBlogs() throws Exception {
        ...
        restBlogMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(blog.getId().toString())))
    ...
    void getNonExistingBlog() throws Exception {
        // Get the blog
        restBlogMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void deleteBlog() throws Exception {
        ...
        // Delete the blog
        restBlogMockMvc
            .perform(delete(ENTITY_API_URL_ID, blog.getId().toString()).with(csrf()).accept(MediaType.APPLICATION_JSON))
