...
import com.saathratri.developer.blog.repository.rowmapper.UserRowMapper;
...
class BlogRepositoryInternalImpl extends SimpleR2dbcRepository<Blog, UUID> implements BlogRepositoryInternal {
    ...
    @Override
    public Mono<Blog> findById(UUID id) {
        Comparison whereClause = Conditions.isEqual(entityTable.column("id"), Conditions.just(StringUtils.wrap(id.toString(), "'")));
        return createQuery(null, whereClause).one();
    }

    @Override
    public Mono<Blog> findOneWithEagerRelationships(UUID id) {
        return findById(id);
    }
