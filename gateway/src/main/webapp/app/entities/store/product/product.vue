<template>
  <div>
    <h2 id="page-heading" data-cy="ProductHeading">
      <span v-text="t$('gatewayApp.storeProduct.home.title')" id="product-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('gatewayApp.storeProduct.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ProductCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('gatewayApp.storeProduct.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && products && products.length === 0">
      <span v-text="t$('gatewayApp.storeProduct.home.notFound')"></span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="t$('gatewayApp.storeProduct.title')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('price')">
              <span v-text="t$('gatewayApp.storeProduct.price')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('image')">
              <span v-text="t$('gatewayApp.storeProduct.image')"></span>
          ...
          <tr v-for="product in products" :key="product.id" data-cy="entityTable">
            ...
            <td>
              <a v-if="product.image" v-on:click="openFile(product.imageContentType, product.image)">
                <img v-bind:src="'data:' + product.imageContentType + ';base64,' + product.image" style="max-height: 30px" alt="product" />
              </a>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="gatewayApp.storeProduct.delete.question" data-cy="productDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-product-heading" v-text="t$('gatewayApp.storeProduct.delete.question', { id: removeId })"></p>
      </div>
