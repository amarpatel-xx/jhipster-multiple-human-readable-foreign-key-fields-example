...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'standard quietly', handle: 'considering huzzah' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('harvester');
      cy.get(`[data-cy="name"]`).should('have.value', 'harvester');

      cy.get(`[data-cy="handle"]`).type('amidst even');
      cy.get(`[data-cy="handle"]`).should('have.value', 'amidst even');
