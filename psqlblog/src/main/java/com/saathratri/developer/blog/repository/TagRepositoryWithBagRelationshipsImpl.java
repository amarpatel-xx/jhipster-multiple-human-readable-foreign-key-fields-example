...
public class TagRepositoryWithBagRelationshipsImpl implements TagRepositoryWithBagRelationships {
    ...
    public Optional<Tag> fetchBagRelationships(Optional<Tag> tag) {
        return tag.map(this::fetchPosts);
    }
    ...
    public List<Tag> fetchBagRelationships(List<Tag> tags) {
        return Optional.of(tags).map(this::fetchPosts).orElse(Collections.emptyList());
    }

    Tag fetchPosts(Tag result) {
        return entityManager
            .createQuery("select tag from Tag tag left join fetch tag.posts where tag.id = :id", Tag.class)
    ...
    List<Tag> fetchPosts(List<Tag> tags) {
        ...
        List<Tag> result = entityManager
            .createQuery("select tag from Tag tag left join fetch tag.posts where tag in :tags", Tag.class)
