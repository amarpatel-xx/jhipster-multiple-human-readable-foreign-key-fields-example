...
public class BlogResource {
    ...
    private static final String ENTITY_NAME = "psqlblogBlog";
    ...
    /**
     * {@code GET  /saathratri-orchestrator} : get all the Blogs.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Blogs in body.
     */
    @GetMapping("/saathratri-orchestrator")
    public ResponseEntity<List<BlogDTO>> getAllBlogsForSaathratriOrchestrator() {
        LOG.debug("REST request to get all Blogs for saathratri orchestrator");
        List<BlogDTO> entityList;
        entityList = blogService.findAllForSaathratriOrchestrator();
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<BlogDTO> getBlog(@PathVariable("id") UUID id) {
